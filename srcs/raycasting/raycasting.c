...
/*   Updated: 2024/12/01 16:04:10 by shmoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_deltadist(t_map *game)
{
...
static void	set_step(t_map *game)
{
	if (game->ray->raydirx < 0)
	{
		game->ray->stepx = -1;
		game->ray->sidedistx = (game->ply->pos_x - (double)game->ray->mapx) * game->ray->deltadistx;
	}
	else
	{
		game->ray->stepx = 1;
		game->ray->sidedistx = (game->ply->pos_x - game->ray->mapx + 1.0) * game->ray->deltadistx;
	}
	if (game->ray->raydiry < 0)
	{
		game->ray->stepy = -1;
		game->ray->sidedisty = (game->ply->pos_y - (double)game->ray->mapy) * game->ray->deltadisty;
	}
	else
	{
		game->ray->stepy = 1;
		game->ray->sidedisty = (game->ply->pos_y - game->ray->mapy + 1.0) * game->ray->deltadisty;
	}
}
