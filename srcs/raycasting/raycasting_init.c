...
/*   Updated: 2024/12/18 12:58:02 by shmoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_raycast(t_game *game)
{
	...
	rad_angle = game->ply->dir_angle * (M_PI / 180.0);
...
void	set_step(t_game *game)
{
	...
	{
		game->ray->stepx = -1;
		game->ray->sidedistx = (game->ply->pos_x - game->ray->mapx)
			* game->ray->deltadistx;
	}
	else
	{
		game->ray->stepx = 1;
		game->ray->sidedistx = (game->ray->mapx + 1.0 - game->ply->pos_x)
			* game->ray->deltadistx;
	}
	if (game->ray->raydiry < 0)
	{
		game->ray->stepy = -1;
		game->ray->sidedisty = (game->ply->pos_y - game->ray->mapy)
			* game->ray->deltadisty;
	}
	else
	{
		game->ray->stepy = 1;
		game->ray->sidedisty = (game->ray->mapy + 1.0 - game->ply->pos_y)
			* game->ray->deltadisty;
	}
}
...
void	ft_init(t_game *game)
{
	...
	game->ray->raydirx = game->ply->dir_x + game->ply->plane_x
		* game->ray->camerax;
	game->ray->raydiry = game->ply->dir_y + game->ply->plane_y
		* game->ray->camerax;
